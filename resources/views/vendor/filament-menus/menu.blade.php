...
">
        @if(isset($item['icon']) && !empty($item['icon']))
            @if(url()->current() === ($item['route']?route($item['route']): $item['url']))
                <x-icon class="w-5 h-5 fi-sidebar-item-icon text-primary-600 dark:text-primary-400" name="{{ $item['icon'] }}"></x-icon>
            @else
                <x-icon class="w-5 h-5 text-gray-700 fi-sidebar-item-icon dark:text-gray-200" name="{{ $item['icon'] }}"></x-icon>
            @endif
        @endif
